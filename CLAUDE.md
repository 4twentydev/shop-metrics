...
- `app/` — Next.js routes. Route groups: `(auth)` (sign-in), `(marketing)` (public landing), `(platform)` (all protected pages). API routes under `app/api/`.
- `features/` — Domain business logic, grouped by feature. Each module contains server actions, queries, Zod schemas, and `components/` sub-folders.
